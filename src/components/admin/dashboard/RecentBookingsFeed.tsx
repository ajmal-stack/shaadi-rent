...
const STATUS_STYLES: Record<string, { bg: string; text: string; border: string; label: string }> = {
  ...
  active: { bg: "bg-emerald-50", text: "text-emerald-800", border: "border-emerald-200", label: "Active" },
  completed: { bg: "bg-stone-100", text: "text-stone-600", border: "border-stone-200", label: "Completed" },
  cancelled: { bg: "bg-rose-50", text: "text-rose-700", border: "border-rose-200", label: "Cancelled" },
  disputed: { bg: "bg-orange-50", text: "text-orange-800", border: "border-orange-200", label: "Disputed" },
  returned: { bg: "bg-teal-50", text: "text-teal-700", border: "border-teal-200", label: "Returned" },
  delivered: { bg: "bg-indigo-50", text: "text-indigo-700", border: "border-indigo-200", label: "Delivered" },
};
...
export function RecentBookingsFeed({ bookings }: RecentBookingsFeedProps) {
  if (!bookings || bookings.length === 0) {
    return (
      <p className="text-xs text-stone-400 py-8 text-center">No bookings yet.</p>
    );
  }
      ...
      {bookings.map((b) => {
        ...
        return (
          ...
          >
            ...
            {/* Amount */}
            <p className="text-xs font-bold text-stone-800 tabular-nums flex-shrink-0">
            ...
            <Link
              href="/admin/bookings"
              className="text-[11px] font-semibold text-rose-800 hover:text-rose-950 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity"
            >
              View →
            </Link>
          </div>
