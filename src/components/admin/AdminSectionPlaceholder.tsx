      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="font-display text-2xl sm:text-3xl font-bold text-stone-900 flex items-center gap-2.5">
              <span className="p-2 rounded-xl bg-stone-900 text-white shadow-sm inline-flex items-center justify-center">
                <Icon size={22} className="text-amber-400" />
              </span>
              <span>{title}</span>
            </h1>
            <p className="mt-1 text-sm text-stone-500 max-w-2xl">{subtitle}</p>
          </div>

          <div className="flex items-center gap-2.5">
            <Link
              href="/admin"
              className="inline-flex items-center gap-2 text-xs font-semibold text-stone-600 hover:text-stone-900 bg-white border border-stone-200 px-3.5 py-2.5 rounded-xl shadow-2xs transition-all hover:bg-stone-50"
            >
              <ArrowLeft size={14} />
              <span>Dashboard</span>
            </Link>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {stats.map((stat, idx) => (
            <div
              key={idx}
              className="rounded-2xl border border-stone-200 bg-white p-5 shadow-2xs"
            >
              ...
              <p className="text-[11px] text-stone-400 mt-1">{stat.helper}</p>
            </div>
          ))}
        </div>

        {/* Informational Hero Card */}
        <div className="rounded-3xl border border-dashed border-stone-300 bg-white/70 p-8 text-center space-y-4">
          <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-rose-50 to-amber-50 text-rose-800 ring-1 ring-rose-200 shadow-inner">
            <Icon size={28} />
          </div>

          <div className="max-w-md mx-auto space-y-1.5">
            <span className="inline-block rounded-full bg-amber-100 text-amber-900 text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 border border-amber-200">
              {badgeText}
            </span>
          ...
          <div className="pt-2 flex justify-center items-center gap-3">
            <Link
              href="/admin/applications"
              className="inline-flex items-center gap-1.5 text-xs font-semibold text-rose-800 hover:text-rose-950 underline underline-offset-4"
            >
