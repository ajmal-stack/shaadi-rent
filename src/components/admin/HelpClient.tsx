    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between">
        <div className="relative flex-1 max-w-md">
          <Search size={15} className="absolute left-3.5 top-3 text-stone-400 dark:text-stone-500" />
          <input
            type="text"
            placeholder="Search standard operating procedures..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full pl-9 pr-3 py-2 text-xs rounded-xl border border-stone-200 dark:border-stone-800 bg-white dark:bg-stone-900 text-stone-800 dark:text-stone-100 placeholder-stone-400 dark:placeholder-stone-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 shadow-xs"
          />
        </div>
      ...
      <div className="rounded-2xl border border-amber-200 dark:border-amber-900/60 bg-amber-50/70 dark:bg-amber-950/20 p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-start gap-3">
          <div className="p-2 rounded-xl bg-amber-100 dark:bg-amber-900/50 text-amber-900 dark:text-amber-300 shrink-0">
            <PhoneCall size={18} />
          </div>
          <div>
            <h4 className="text-sm font-bold text-amber-950 dark:text-amber-200">Urgent Owner Escalation Desk</h4>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className="text-xs font-mono font-semibold bg-white dark:bg-stone-900 border border-amber-200 dark:border-amber-800/80 px-3 py-1.5 rounded-lg text-amber-950 dark:text-amber-200">
            +91 98765 43210
          </span>
          <span className="text-[11px] text-amber-700 dark:text-amber-400 font-medium">(24/7 Priority)</span>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {filtered.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className="rounded-2xl border border-stone-200 dark:border-stone-800 bg-white dark:bg-stone-900 shadow-xs overflow-hidden transition-all"
            >
              <button
                onClick={() => toggleExpand(item.id)}
                className="w-full text-left p-5 flex items-start justify-between gap-4 hover:bg-stone-50/60 dark:hover:bg-stone-800/40 transition-colors"
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] font-bold uppercase tracking-wider text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200/60 dark:border-emerald-800/60 px-2 py-0.5 rounded-md">
                      {item.categoryLabel}
                    </span>
                  </div>
                  <h3 className="text-sm sm:text-base font-bold text-stone-900 dark:text-stone-100">{item.title}</h3>
                  <p className="text-xs text-stone-500 dark:text-stone-400 leading-relaxed">{item.summary}</p>
                </div>
                <div className="p-1 text-stone-400 dark:text-stone-500 shrink-0 mt-1">
                  {isExpanded ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
                </div>
              </button>

              {isExpanded && (
                <div className="px-5 pb-5 pt-2 border-t border-stone-100 dark:border-stone-800 space-y-4 text-xs">
                  <div>
                    <h5 className="font-semibold text-stone-800 dark:text-stone-200 mb-2 uppercase tracking-wide text-[11px]">
                      Step-by-Step Procedure
                    </h5>
                    <ol className="space-y-2 list-decimal list-inside text-stone-600 dark:text-stone-400 leading-relaxed">
                      {item.steps.map((step, idx) => (
                        <li key={idx} className="pl-1">
                          <span className="font-medium text-stone-700 dark:text-stone-300">{step}</span>
                        </li>
                      ))}
                    </ol>
                  </div>
                  ...
                  {item.tips && item.tips.length > 0 && (
                    <div className="rounded-xl bg-stone-50 dark:bg-stone-800/60 border border-stone-200/80 dark:border-stone-700/80 p-3.5 space-y-1.5">
                      ...
                      <ul className="space-y-1 list-disc list-inside text-stone-600 dark:text-stone-400 pl-1">
                  ...
                  {item.alert && (
                    <div className="rounded-xl bg-rose-50 dark:bg-rose-950/30 border border-rose-200 dark:border-rose-900/60 p-3 flex items-start gap-2 text-rose-800 dark:text-rose-300 font-medium">
        ...
        {filtered.length === 0 && (
          <div className="p-12 text-center bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-800 text-stone-400 dark:text-stone-500">
            <HelpCircle size={36} className="mx-auto mb-2 text-stone-300 dark:text-stone-600" />
            <p className="text-sm font-semibold text-stone-700 dark:text-stone-300">No matching SOP found</p>
            <p className="text-xs text-stone-400 dark:text-stone-500 mt-1">Try another search query or category filter</p>
          </div>
        )}
      </div>
    </div>
